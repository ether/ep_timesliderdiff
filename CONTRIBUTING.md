# Contributing to ep_timesliderdiff

Thanks for helping improve `ep_timesliderdiff` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_timesliderdiff` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_timesliderdiff from your local clone
pnpm run plugins i --path ../ep_timesliderdiff

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_timesliderdiff/` are picked up after a server restart.
...
* Etherpad version, ep_timesliderdiff version, browser + OS
* Steps to reproduce
